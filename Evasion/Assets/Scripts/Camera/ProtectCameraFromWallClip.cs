...
namespace Game.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        [HideInInspector] public float m_OriginalDist;             // the original distance to the camera before any modification are made
        private float m_MoveVelocity;             // the velocity at which the camera moved
        [HideInInspector] public float m_CurrentDist;              // the current distance from the camera to the target
        ...
        private void LateUpdate()
        {
            ...
            if (hitSomething)
            {
                Draw.DrawRay(m_Ray.origin, -m_Pivot.forward*(targetDist + sphereCastRadius), Color.magenta);
            }
